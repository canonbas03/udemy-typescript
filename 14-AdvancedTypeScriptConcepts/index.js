...
let str = value; // No error, but incorrect
...
const newUser = user; // No error, even though isAdmin is not in User
console.log(newUser); // Works, but isAdmin is silently ignored
let handleAnimal = (animal) => {
...
function getArea(shape) {
    switch (shape.kind) {
        ...
        case "square":
            return shape.side ** 2;
        default:
            const _exhaustiveCheck = shape; // TypeScript error if a case is missing
